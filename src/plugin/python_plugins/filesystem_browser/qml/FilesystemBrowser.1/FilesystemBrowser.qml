    ...
    SplitView {
        ...
        RowLayout {
            ...
            Rectangle {
                ...
                Layout.preferredHeight: rowHeight
                color: upMouse.containsMouse ? "#444444" : "#333333"
                radius: 3
                border.color: "#555555"
                border.width: 1
                ...
                MouseArea {
                    ...
                    onClicked: {
                        var cur = current_path_attr.value || "/";
                        cur = cur.replace(/\\/g, "/");
                        // Strip trailing slash (but not for drive roots like "X:/")
                        if (cur.length > 1 && cur.charAt(cur.length - 1) === '/' && cur.charAt(cur.length - 2) !== ':') {
                            cur = cur.substring(0, cur.length - 1);
                        }
                        var lastSlash = cur.lastIndexOf("/");
                        if (lastSlash > 0) {
                            var parent_path = cur.substring(0, lastSlash);
                            // If we'd end up at just "X:", add the slash back
                            if (parent_path.length === 2 && parent_path.charAt(1) === ':') {
                                parent_path += "/";
                            }
                            sendCommand({"action": "change_path", "path": parent_path});
                        } else if (lastSlash === 0) {
                            // We're at a top-level Unix path, go to root
                            sendCommand({"action": "change_path", "path": "/"});
                        } else {
                            // Drive root like "X:/" — go to virtual root
                            sendCommand({"action": "change_path", "path": "/"});
                        }
                    }
                }
                ...
                ToolTip.text: "Go up one directory"
                ToolTip.delay: 500
            }
             ...
             ComboBox {
              ...
              RowLayout {
                   ...
                   SpinBox {
                       ...
                       from: 0
                       to: 10
                       value: depth_limit_attr.value !== undefined ? parseInt(depth_limit_attr.value) : 0
                       editable: true
